...
class InterferenceCorrector(nnx.Module):
    def __init__(
        ...
        *,
        ...
        # Encoder
        self.enc_conv0 = ConvBlock(
            1, 16, (3, 3), padding=enc_pad, rngs=rngs
        )  # (192, 384, 16)
        self.pool0 = partial(
        ...
        self.enc_conv1 = ConvBlock(
            16, 32, (3, 3), padding=enc_pad, rngs=rngs
        )  # (64, 64, 32) -> s1
        self.pool1 = partial(
            nnx.max_pool, window_shape=(4, 4), strides=(4, 4)
        )  # (16, 16, 32)

        self.enc_conv2 = ConvBlock(
            32, 64, (3, 3), padding=enc_pad, rngs=rngs
        )  # (16, 16, 64) -> s2
        self.pool2 = partial(
            nnx.max_pool, window_shape=(2, 2), strides=(2, 2)
        )  # (8, 8, 64)

        self.enc_conv3 = ConvBlock(
            64, 128, (3, 3), padding=enc_pad, rngs=rngs
        )  # (8, 8, 128) -> s3
        self.pool3 = partial(
            nnx.max_pool, window_shape=(2, 2), strides=(2, 2)
        )  # (4, 4, 128)
        ...
        # Decoder
        # Upsample from (4,4,128) from bottleneck
        self.dec_conv0 = ConvBlock(128, 64, (3, 3), rngs=rngs)  # Out: (4,4,64)
        self.upsample0 = partial(
            resize_batch, shape=(8, 8, 64), method="bilinear"
        )  # Out: (8,8,64)
        # s3 is (8,8,128). Concatenated channels: 64 + 128 = 192
        self.dec_conv1 = ConvBlock(192, 64, (3, 3), rngs=rngs)  # Out: (8,8,64)

        self.upsample1 = partial(
            resize_batch, shape=(16, 16, 64), method="bilinear"
        )  # Out: (16,16,64)
        # s2 is (16,16,64). Concatenated channels: 64 + 64 = 128
        self.dec_conv2 = ConvBlock(128, 32, (3, 3), rngs=rngs)  # Out: (16,16,32)

        # Further processing to get to 16 channels before final 1x1 conv
        self.dec_conv3 = ConvBlock(32, 16, (3, 3), rngs=rngs)  # Out: (16,16,16)
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        s1 = self.enc_conv1(
            p0
        )  # Skip connection 1: (64, 64, 32) - Not used in this config for 16x16 output
        p1 = self.pool1(s1)

        s2 = self.enc_conv2(p1)  # Skip connection 2: (16, 16, 64)
        p2 = self.pool2(s2)

        s3 = self.enc_conv3(p2)  # Skip connection 3: (8, 8, 128)
        p3 = self.pool3(s3)

        # Bottleneck
        b = self.bottleneck(p3)  # (4, 4, 128)

        # Decoder Path
        d0 = self.dec_conv0(b)  # (4,4,64)
        u0 = self.upsample0(d0)  # (8,8,64)

        # Concatenate with skip connection s3
        c0 = jnp.concatenate([u0, s3], axis=-1)  # (8,8, 64+128=192)
        d1 = self.dec_conv1(c0)  # (8,8,64)

        u1 = self.upsample1(d1)  # (16,16,64)
        # Concatenate with skip connection s2
        c1 = jnp.concatenate([u1, s2], axis=-1)  # (16,16, 64+64=128)
        d2 = self.dec_conv2(c1)  # (16,16,32)

        d3 = self.dec_conv3(d2)  # (16,16,16)
